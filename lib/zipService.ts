...
export const createAndDownloadZip = async (
  ...
  for (const stamp of stamps) {
    ...
    if (stamp.isAnimated) {
      // Compiled dataUrl contains the fully composed APNG
      ...
      blob = await createFinalImageBlob(stamp.dataUrl, config, TARGET_WIDTH, TARGET_HEIGHT);
    }
    if (!blob) continue;
...
export async function loadProjectFromZip(file: File): Promise<{
  ...
  if (projectFile) {
    ...
    const restoredStamps: Stamp[] = (projectData.stamps || []).map((s: any, idx: number) => ({
      ...
      rawOriginalFrames: s.rawOriginalFrames ?? undefined,
      textObjectsFrames: s.textObjectsFrames ? s.textObjectsFrames.map((framesList: any[]) => 
        (framesList ?? []).map((t: any, i: number) => ({
          ...t,
          layerOrder: t.layerOrder ?? (t.zIndex === 'back' ? 10 + i : 150 + i),
          outlineColor: t.outlineColor ?? '#ffffff',
          outlineWidth: t.outlineWidth ?? 4,
      ...
      imageLayersFrames: s.imageLayersFrames ? s.imageLayersFrames.map((framesList: any[]) => 
        (framesList ?? []).map((l: any, i: number) => ({
          ...l,
          layerOrder: l.layerOrder ?? (l.zIndex === 'back' ? 30 + i : 170 + i),
        }))
      ) : undefined,
      drawingStrokesFrames: s.drawingStrokesFrames ? s.drawingStrokesFrames.map((framesList: any[]) => 
        (framesList ?? []).map((d: any, i: number) => ({
          ...d,
          layerOrder: d.layerOrder ?? (d.zIndex === 'back' ? 20 + i : 160 + i),
          outlineColor: d.outlineColor ?? '#ffffff',
          outlineWidth: d.outlineWidth ?? 0,
    ...
    const sanitizeConfig = (config: any): ExportConfig | null => {
      ...
      return {
        ...
        rawOriginalFrames: config.rawOriginalFrames ?? undefined,
        textObjectsFrames: config.textObjectsFrames ? config.textObjectsFrames.map((framesList: any[]) => 
          (framesList ?? []).map((t: any, i: number) => ({
            ...t,
            layerOrder: t.layerOrder ?? (t.zIndex === 'back' ? 10 + i : 150 + i),
            outlineColor: t.outlineColor ?? '#ffffff',
        ...
        imageLayersFrames: config.imageLayersFrames ? config.imageLayersFrames.map((framesList: any[]) => 
          (framesList ?? []).map((l: any, i: number) => ({
            ...l,
            layerOrder: l.layerOrder ?? (l.zIndex === 'back' ? 30 + i : 170 + i),
          }))
        ) : undefined,
        drawingStrokesFrames: config.drawingStrokesFrames ? config.drawingStrokesFrames.map((framesList: any[]) => 
          (framesList ?? []).map((d: any, i: number) => ({
            ...d,
            layerOrder: d.layerOrder ?? (d.zIndex === 'back' ? 20 + i : 160 + i),
            outlineColor: d.outlineColor ?? '#ffffff',
